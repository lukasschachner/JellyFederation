...
{
    [LoggerMessage(1, LogLevel.Debug, "Replacing stale connection for server {ServerId}: {OldConnectionId} -> {NewConnectionId}")]
    private static partial void LogReplacingStaleConnection(ILogger logger, Guid serverId, string oldConnectionId, string newConnectionId);

    [LoggerMessage(2, LogLevel.Information, "Registered connection {ConnectionId} for server {ServerId} from {PublicIp}")]
    private static partial void LogRegisteredConnection(ILogger logger, Guid serverId, string connectionId, IPAddress publicIp);
    ...
    [LoggerMessage(7, LogLevel.Debug, "Staged hole punch candidate for request {RequestId}: server={ServerId}, port={UdpPort}, candidates={CandidateCount}")]
    private static partial void LogHolePunchStaged(ILogger logger, Guid requestId, Guid serverId, int udpPort, int candidateCount);

    [LoggerMessage(8, LogLevel.Information, "Hole punch request {RequestId} ready to dispatch with {CandidateCount} candidate(s)")]
