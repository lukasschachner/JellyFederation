...
public partial class InvitationsController
{
    ...
    [LoggerMessage(3, LogLevel.Information,
        "Invitation relationship already exists between {FromServerId} and {ToServerId}")]
    ...
    private static partial void LogInvitationCreated(ILogger logger, Guid invitationId, Guid fromServerId,
        Guid toServerId);
    ...
    [LoggerMessage(7, LogLevel.Information,
        "Invitation {InvitationId} cannot be responded to because status is {Status}")]
    private static partial void LogInvitationRespondNotPending(ILogger logger, Guid invitationId,
