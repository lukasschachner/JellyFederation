...
public partial class FileRequestsController
{
    ...
    [LoggerMessage(1, LogLevel.Information,
        "Create file request from {RequestingServerId} to {OwningServerId} for item {ItemId}")]
    private static partial void LogCreateRequested(ILogger logger, Guid requestingServerId, Guid owningServerId,
        string itemId);

    [LoggerMessage(2, LogLevel.Warning,
        "Create file request failed: owning server {OwningServerId} not found (requesting {RequestingServerId})")]
    private static partial void LogCreateOwningServerNotFound(ILogger logger, Guid owningServerId,
        Guid requestingServerId);

    [LoggerMessage(3, LogLevel.Warning,
        "Create file request forbidden: no accepted invitation between {RequestingServerId} and {OwningServerId}")]
    private static partial void LogCreateForbiddenNoInvitation(ILogger logger, Guid requestingServerId,
        Guid owningServerId);

    [LoggerMessage(4, LogLevel.Information,
        "Created file request {RequestId} from {RequestingServerId} to {OwningServerId}")]
    private static partial void LogCreated(ILogger logger, Guid requestId, Guid requestingServerId,
        Guid owningServerId);

    [LoggerMessage(5, LogLevel.Debug,
        "Owning plugin offline while creating request {RequestId} for server {OwningServerId}")]
    ...
    [LoggerMessage(6, LogLevel.Debug,
        "Requesting plugin offline while creating request {RequestId} for server {RequestingServerId}")]
    private static partial void
        LogCreateRequesterPluginOffline(ILogger logger, Guid requestId, Guid requestingServerId);
    ...
    [LoggerMessage(9, LogLevel.Warning,
        "Cancel file request {RequestId} forbidden for server {ServerId} (requesting={RequestingServerId}, owning={OwningServerId})")]
    private static partial void LogCancelForbidden(ILogger logger, Guid requestId, Guid serverId,
        Guid requestingServerId, Guid owningServerId);
    ...
    [LoggerMessage(12, LogLevel.Warning,
        "Mark complete failed: file request {RequestId} not found for server {ServerId}")]
    ...
    [LoggerMessage(13, LogLevel.Warning,
        "Mark complete forbidden: file request {RequestId} by server {ServerId} (requesting={RequestingServerId})")]
    private static partial void LogMarkCompleteForbidden(ILogger logger, Guid requestId, Guid serverId,
        Guid requestingServerId);
