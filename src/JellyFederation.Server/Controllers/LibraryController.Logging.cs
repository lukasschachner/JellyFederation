...
public partial class LibraryController
{
    ...
    [LoggerMessage(1, LogLevel.Information,
        "Library sync started for {ServerId}: incoming={IncomingCount}, replaceAll={ReplaceAll}")]
    private static partial void LogSyncStarted(ILogger logger, Guid serverId, int incomingCount, bool replaceAll);

    [LoggerMessage(2, LogLevel.Information,
        "Library sync completed for {ServerId}: incoming={IncomingCount}, added={Added}, updated={Updated}, removed={Removed}")]
    private static partial void LogSyncCompleted(ILogger logger, Guid serverId, int incomingCount, int added,
        int updated, int removed);
    ...
    [LoggerMessage(4, LogLevel.Debug,
        "Mine returned for {ServerId}: total={Total}, page={Page}, pageSize={PageSize}, search='{Search}', type='{Type}'")]
    private static partial void LogMineReturned(ILogger logger, Guid serverId, int total, int page, int pageSize,
    ...
    private static partial void
        LogSetRequestableUpdated(ILogger logger, Guid itemId, Guid serverId, bool isRequestable);

    [LoggerMessage(8, LogLevel.Debug,
        "Browse returned for {ServerId}: total={Total}, page={Page}, pageSize={PageSize}, search='{Search}', type='{Type}'")]
    private static partial void LogBrowseReturned(ILogger logger, Guid serverId, int total, int page, int pageSize,
        string? search, string? type);
